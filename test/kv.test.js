...
// 脚本化 Upstash REST：按命令队列应答，记录实际发出的命令（兼容 @upstash/redis 管道与标准 REST）
function scriptUpstash({ get = {}, putOk = true, hang = false } = {}) {
  ...
  globalThis.fetch = async (_url, { body, signal } = {}) => {
    ...
    const cmds = Array.isArray(raw[0]) ? raw : [raw];
    const results = [];

    for (const c of cmds) {
      const normalizedCmd = c.map((part, idx) => {
        if (typeof part === "string" && (idx === 0 || part.toLowerCase() === "ex")) {
      ...
      const verb = normalizedCmd[0];
      if (verb === "GET") {
        const v = Object.hasOwn(get, normalizedCmd[1]) ? get[normalizedCmd[1]] : null;
    ...
    const respBody = Array.isArray(raw[0]) ? results : results[0];
    return new Response(JSON.stringify(respBody), { status: 200 });
  };
  return commands;
}
...
test("upstash adapter reads remote, falls back to memory on failure", async () => {
  ...
  const kv = createUpstashKv({ url: "https://mock.upstash.io", token: "t", fallback: mem });
  ...
  const kv2 = createUpstashKv({ url: "https://mock.upstash.io", token: "t", fallback: mem });
  assert.equal(await kv2.get("fb"), "mem-v");
});

test("upstash put honors expirationTtl via SET EX", async () => {
  const commands = scriptUpstash({});
  const kv = createUpstashKv({ url: "https://mock.upstash.io", token: "t", fallback: createMemoryKv() });
...
test("upstash get degrades to memory within timeout budget", async () => {
  ...
  const kv = createUpstashKv({ url: "https://mock.upstash.io", token: "t", timeoutMs: 50, fallback: mem });
  const t0 = Date.now();
...
test("createKvFromEnv selects memory without credentials, composite with them", async () => {
  ...
  const comp = createKvFromEnv({ UPSTASH_REDIS_REST_URL: "https://mock.upstash.io", UPSTASH_REDIS_REST_TOKEN: "t" });
...
test("getWithStatus: remote miss vs remote failure are distinguishable", async () => {
  ...
  const kv2 = createUpstashKv({ url: "https://mock.upstash.io", token: "t", fallback: createMemoryKv() });
